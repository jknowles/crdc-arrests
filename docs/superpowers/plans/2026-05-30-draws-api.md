...
test_that("build_district_dim unions years, deduplicates by LEAID, and maps FIPS to state", {
  ...
  # state mapped from FIPS "01" -> AL -> Alabama
...
build_district_dim <- function(ccd_list) {
  stopifnot(
    is.list(ccd_list),
    length(ccd_list) > 0,
    all(vapply(ccd_list, is.data.frame, logical(1)))  # reject a bare data.frame
  )
  combined <- do.call(rbind, lapply(ccd_list, function(d) {
    data.frame(
      # zero-pad (formatC + integer), NOT sprintf("%07s") which space-pads
  ...
  district_dim_df <- district_dim_df[, c("LEAID", "lea_name", "LEA_STATE",
                 "state_name", "lat", "lon", "enrollment")]
