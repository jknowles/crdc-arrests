...
# Builds unified_m1_mod and its upstream data-prep targets. Requires CmdStan (run
# setup.R first). NEVER run this in CI — it fits a model.
set -euo pipefail
cd "$(dirname "$0")/.."

export CRDC_DEV_MODE=true
echo "== DEV_MODE smoke: building unified_m1_mod (+ upstream) with reduced iters/enroll cap =="
echo "   (expect a few minutes once CmdStan is set up; this proves the pipeline runs)"
Rscript -e 'targets::tar_make(names = "unified_m1_mod")'
echo "== Smoke build complete. Inspect with: Rscript -e 'targets::tar_read(unified_m1_mod)' =="
