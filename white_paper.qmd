...
# --- Reproducible 2021-22 national descriptives ------------------------------
# Every arrest count / rate reported in the Data section below is regenerated
# here from the same full CRDC wave (crdc_2122) the rest of the paper uses, so
# the inline prose numbers stay in sync with the source data instead of being
# hand-entered.
.tot <- crdc_2122 |> dplyr::filter(RACE == "TOTAL", SEX == "TOTAL") |>
  dplyr::summarize(arrests = sum(ARRESTS), enroll = sum(stu_enroll),
                   leas = dplyr::n_distinct(LEAID),
                   leas_arr = dplyr::n_distinct(LEAID[ARRESTS > 0]))
nat_arrests <- .tot$arrests; nat_enroll <- .tot$enroll
nat_rate <- nat_arrests / (nat_enroll / 1000)
n_leas <- .tot$leas; n_leas_arr <- .tot$leas_arr
pct_leas_arr <- 100 * n_leas_arr / n_leas

.byrace <- crdc_2122 |> dplyr::filter(RACE != "TOTAL", SEX != "TOTAL") |>
  dplyr::group_by(RACE) |>
  dplyr::summarize(enroll = sum(stu_enroll), arrests = sum(ARRESTS), .groups = "drop") |>
  dplyr::mutate(rate = arrests / (enroll / 1000))
race_arr  <- stats::setNames(.byrace$arrests, .byrace$RACE)
race_enr  <- stats::setNames(.byrace$enroll,  .byrace$RACE)
race_rate <- stats::setNames(.byrace$rate,    .byrace$RACE)

.bysex <- crdc_2122 |> dplyr::filter(RACE != "TOTAL", SEX != "TOTAL") |>
  dplyr::group_by(SEX) |>
  dplyr::summarize(enroll = sum(stu_enroll), arrests = sum(ARRESTS), .groups = "drop") |>
  dplyr::mutate(rate = arrests / (enroll / 1000))
sex_arr  <- stats::setNames(.bysex$arrests, .bysex$SEX)
sex_rate <- stats::setNames(.bysex$rate,    .bysex$SEX)

.byrs <- crdc_2122 |> dplyr::filter(RACE != "TOTAL", SEX != "TOTAL") |>
  dplyr::group_by(RACE, SEX) |>
  dplyr::summarize(enroll = sum(stu_enroll), arrests = sum(ARRESTS), .groups = "drop") |>
  dplyr::mutate(rate = arrests / (enroll / 1000))
blm_rate <- .byrs$rate[.byrs$RACE == "BL" & .byrs$SEX == "M"]
whf_rate <- .byrs$rate[.byrs$RACE == "WH" & .byrs$SEX == "F"]

# inline formatters for the Data-section prose
.cnt <- function(x) format(round(x), big.mark = ",")
.rt  <- function(x) sprintf("%.2f", x)
```
...
The CRDC is generally a biennial data collection. The most recent available data, released in January 2025, are for the 2021-22 school year. To this, we add data from two prior collections: 2017-18 and 2015-16.[^2] The CRDC includes `r .cnt(n_leas)` school districts in 2021-22; 89% of these districts have three years of data available. In models that incorporate multiple waves of data, we include all available observations, which is three years for most districts but one or two years in some cases.[^3] The number of school-related arrests declined over these three waves from over 62,000 school-related arrests in 2015-16 to 52,300 in 2017-18 to `r .cnt(nat_arrests)` in 2021-22.

Student arrests are rare. In 2021-22, `r .cnt(nat_arrests)` students were arrested at school, including `r .cnt(race_arr[["WH"]])` White students; `r .cnt(race_arr[["BL"]])` Black students; `r .cnt(race_arr[["HI"]])` Hispanic students; and `r .cnt(race_arr[["AM"]])` American Indian/Alaskan Native students.[^4] More Black students were arrested than White students despite there being three times as many White students as Black students enrolled in U.S. public schools. `r .cnt(sex_arr[["M"]])` male students were arrested along with `r .cnt(sex_arr[["F"]])` female students.

In addition to being rare, student arrests are also sparse. In 2021-22, `r .cnt(n_leas_arr)` districts reported an arrest. This is only `r sprintf("%.1f", pct_leas_arr)`% of the districts in the data, which means the most common value for arrests, by far, is 0. However, this small proportion of districts enrolled 45% of all students -- meaning that nearly half of all students attended a school district where a school-based arrest occurred.
...
The result is a dataset with `r sprintf("%.1f", nat_enroll/1e6)` million students enrolled in the schools included in the CRDC in 2021-22 and a national school-related arrest rate of `r .rt(nat_rate)` out of every 1,000 students. White students had `r .rt(race_rate[["WH"]])` arrests per 1,000 students, Hispanic students `r .rt(race_rate[["HI"]])`, American Indian students `r .rt(race_rate[["AM"]])`, and Black students `r .rt(race_rate[["BL"]])` arrests per 1,000 students. Male students had an arrest rate of `r .rt(sex_rate[["M"]])` arrests per 1,000 while female students were at `r .rt(sex_rate[["F"]])`. The difference in the arrest rates of male and female students (`r .rt(sex_rate[["M"]] - sex_rate[["F"]])` difference) is smaller than the difference in arrests rates of White students and Black or American Indian students (differences of `r .rt(race_rate[["BL"]] - race_rate[["WH"]])` and `r .rt(race_rate[["AM"]] - race_rate[["WH"]])`, respectively). Disaggregating by race and sex reveals even greater disparities: rates by student group vary from a high of `r .rt(blm_rate)` out of 1,000 Black male students to a low of `r .rt(whf_rate)` out of 1,000 White female students. These are shown in Figure 1 below.
...
cov_obs |>
  ...
  dplyr::mutate(
    form = ifelse(startsWith(model_id, "unified"), "Unified", "Stratified"),
    num  = as.integer(sub("_mod$", "", sub("^(unified|stratified)_m", "", model_id)))) |>
...
Number of rows of data = `r format(n_full, big.mark=",")`. Observed precision = 0.384 calculated from the Agresti-Coull approximate interval with the rule of three applied.
...
```{r}
#| label: tbl-coverage-arrest
#| tbl-cap: "Table 4: Model results for sample of district-student groups with an observed arrest"
cov_obs |>
  dplyr::filter(ARRESTS > 0) |>
  dplyr::group_by(model_id) |>
  dplyr::summarize(
    Coverage = 100 * mean(covered),
    precision = mean(fit_precision),
...
Number of rows of data = `r format(sum(cov_obs$ARRESTS[cov_obs$model_id == cov_obs$model_id[1]] > 0), big.mark=",")`. Observed precision = 1.89 calculated from the Agresti-Coull approximate interval.
...
```{r}
#| label: tbl-coverage-most
#| tbl-cap: "Table 5: Model results for 100 districts with most total arrests"
cov_obs |>
  dplyr::filter(LEAID %in% bigarr_ids) |>
  dplyr::group_by(model_id) |>
  dplyr::summarize(
    Coverage = 100 * mean(covered),
    precision = mean(fit_precision),
...
Number of rows of data = `r format(sum(cov_obs$LEAID[cov_obs$model_id == cov_obs$model_id[1]] %in% bigarr_ids), big.mark=",")`. Observed precision = 2.74 calculated from the Agresti-Coull approximate interval with the rule of three applied.
...
noarr_ids <- rdata |>
  dplyr::filter(YEAR == "21-22") |>
  dplyr::group_by(LEAID) |>
  dplyr::summarize(ta = sum(ARRESTS), te = sum(stu_enroll), .groups = "drop") |>
  dplyr::filter(ta == 0) |>
  dplyr::arrange(dplyr::desc(te)) |>
  dplyr::slice_head(n = 100) |>
  dplyr::pull(LEAID)

noarr_draws <- get_prediction_draws(con, LEAID = noarr_ids, YEAR = "21-22")
n_noarr_groups <- dplyr::n_distinct(noarr_draws$LEAID, noarr_draws$RACE, noarr_draws$SEX)

noarr_draws |>
  dplyr::group_by(model_id, draw_id) |>
  dplyr::summarize(tot = sum(pred), .groups = "drop") |>          # arrests summed over groups, per draw
  dplyr::group_by(model_id) |>
  dplyr::summarize(
    modeled = median(tot),
  ...
  dplyr::transmute(
    Models = paste(form, num),
    `Modeled arrests` = format(round(modeled), big.mark = ","),
    `95% expected range` = sprintf("%s -- %s", format(round(lo), big.mark = ","),
                                   format(round(hi), big.mark = ","))) |>
  knitr::kable(align = c("l", "r", "r"))
```

Number of districts = `r length(noarr_ids)` (`r format(n_noarr_groups, big.mark=",")` district-student groups).
